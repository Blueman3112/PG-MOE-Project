...
def create_dataloaders(dataset_path, batch_size=32, num_workers=4):
    ...
    val_dataset = datasets.ImageFolder(root=os.path.join(dataset_path, "val"), transform=val_transform)
    test_dataset = datasets.ImageFolder(root=os.path.join(dataset_path, "test"), transform=val_transform)

    print(f"训练集信息: 共 {len(train_dataset)} 个样本。 (已启用数据增强)")
    print(f"验证集信息: 共 {len(val_dataset)} 个样本。")
    print(f"测试集信息: 共 {len(test_dataset)} 个样本。")
