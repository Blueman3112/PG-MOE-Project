...
from model_fastnew import PGMoEFast  # <--- 修改为采用新的 model_fastnew
from train import setup_logging, calculate_metrics
from loss import OrthogonalLoss

def get_args():
    parser = argparse.ArgumentParser(description="PG-MoE FAST Training Script (New)")
    ...
    parser.add_argument("--fusion_type", type=str, default="moe", 
                        choices=['moe', 'concat', 'spatial_only', 'frequency_only', 'add'], 
                        help="Fusion mechanism to use")
...
def run():
    ...
    # 强制将非 moe 模型的 lambda_orth 设置为 0，因为其他基线网络没有理由加入特征正交损失
    ...
    for epoch in range(args.epochs):
        ...
        for patch_tokens, class_tokens, labels in loader_bar:
            ...
            # 单模态没有 F_s 只有 F_f，或者反过来。Orthogonal loss 需要两个，所以单模态下我们直接略过或者 loss 里防崩溃。
            # 这里由于 lambda_orth 被强制为 0.0，OrthogonalLoss 内部计算出来也是 0 或者不报错，但我们安全起见：
    ...
    with open(info_file, "w") as f:
        ...
        f.write(f"Fast Mode: True (Offline Features, NEW)\n")
    ...
    # ---------------- 重命名结果文件夹为最终格式 ----------------
    final_folder_name = f"fast-{DATASET_NAME}_AUC{best_val_auc:.4f}_ACC{best_val_acc:.4f}_{run_id}_{args.fusion_type}"
    FINAL_OUTPUT_DIR = os.path.join(args.results_dir, final_folder_name)
    
    try:
        os.rename(OUTPUT_DIR, FINAL_OUTPUT_DIR)
        logging.info(f"文件夹已重命名为: {FINAL_OUTPUT_DIR}")
