...
csv_file = os.path.join(script_dir, '表5-5fixed.csv')
output_path = os.path.join(script_dir, '图5-5fixed_grouped_bar.png')
...
# 按照PG-MoE优先的展示顺序排列
method_order = ['PG-MoE', 'noOrth']
df_subset = df_subset.reindex(method_order)

# 3. 颜色及绘图参数设置
colors = ['#E44D35', '#F2A021', '#2DC86E'] # 对应的基础色：红, 橙, 绿
alphas = [1.0, 0.9] # 对应柱体的透明度

x = np.arange(len(method_order))
width = 0.22 # 单个柱子的宽度

fig, ax = plt.subplots(figsize=(8, 6), dpi=300) # 这里柱子少，适当调小一点宽度
...
# 5. 为图例创建带黑色外框的1.0透明度占位方块，放入Legend
for i, metric in enumerate(metrics):
...
ax.set_ylim(90, 105)
ax.set_yticks(np.arange(90, 106, 5))
