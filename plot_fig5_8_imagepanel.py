...
legend_patches = [
    mpatches.Patch(color='#1f77b4', label='空间专家权重 (w_s)'),
    mpatches.Patch(color='#ff7f0e', label='频域专家权重 (w_f)')
]
fig.legend(handles=legend_patches, prop=my_font, loc='upper center', bbox_to_anchor=(0.5, 0.92), ncol=2, fontsize=12, frameon=False)

# 保存
save_path = 'results/Fig5-8_ImagePanel_V3.png'
plt.savefig(save_path, bbox_inches='tight')
print(f'Done! Saved Figure 5-8 Panel to {save_path}')
